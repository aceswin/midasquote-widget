/*
 * MidasQuote COUNTERTOP STORES Widget — forked from widget.js v3.3
 * Countertop-only fork: the cabinet box-building flow ('c'/'b' prefixes —
 * box materials, door styles, drawers, hinges, tall cabinets, crown/valance
 * trim) has been removed entirely. The countertop configurator ('ct') is
 * now the widget's ONLY flow, and every project type leads straight into
 * it — see the "Project basics" step inside buildWidgetHTML's single
 * countertop section. The countertop material engine, specialty items
 * engine, and all shared infrastructure (measurement calculator, lead
 * capture, branding, sticky bar, PDF/email hooks) are unchanged.
 */
...
//This is the widget test file
  ...
  async function loadShopData(token) {
    ...
    // project types for every shop that hasn't touched this new setting yet.
    //
    // COUNTERTOP FORK: replaced the original cabinet-shop defaults (Kitchen,
    // Bathroom, Laundry room, Garage, Commercial, Other, Refacing,
    // Repainting, Restaining) with a countertop-relevant set. Kitchen/
    // Bathroom keep their original photo URLs and copy style. Outdoor
    // Kitchen/BBQ and Bar/Island are new — no real countertop-specific
    // photos exist yet, so they reuse the Kitchen cover image as a
    // clearly-flagged placeholder (see TODOs below) and omit measureImage
    // entirely (the built-in text guide is used instead) rather than reuse
    // a cabinet-measuring photo that wouldn't apply. Commercial/Other keep
    // their original photos too.
    let roomTypes = [];
    try { roomTypes = shop['Room types'] ? JSON.parse(shop['Room types']) : []; } catch(e) { roomTypes = []; }
    if (!Array.isArray(roomTypes) || !roomTypes.length) {
      roomTypes = [
        { id:'kitchen', name:'Kitchen',        adjustment:0,  description:'The kitchen is where life happens — let\'s find the perfect countertop to match how you live and cook.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/kitchen1.jpg' },
        { id:'bathroom',name:'Bathroom',       adjustment:-5, description:'Turn your bathroom into a personal retreat. Choose a vanity top that makes getting ready each morning feel a little more special.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/bathroom.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/bathroom1.jpg' },
        // TODO: placeholder — reuses the Kitchen cover photo. Replace with a real Outdoor Kitchen/BBQ photo when available.
        { id:'outdoor_kitchen', name:'Outdoor Kitchen / BBQ', adjustment:0, description:'Built for the elements and built to entertain — get a durable countertop made for your outdoor kitchen or BBQ station.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg' },
        // TODO: placeholder — reuses the Kitchen cover photo. Replace with a real Bar/Island photo when available.
        { id:'bar_island', name:'Bar / Island', adjustment:0, description:'Whether it\'s a kitchen island or a home bar, get a countertop that becomes the centerpiece of the room.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg' },
        { id:'commercial', name:'Commercial',  adjustment:0,  description:'Make a great first impression. Get commercial-grade countertops built to fit your business, whether it\'s a sleek office or a welcoming retail space.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/commercial.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/commercial1.jpg' },
    ...
    // COUNTERTOP FORK: the cabinet-only line-item categories (material,
    // door, drawer, hinge, install, tax, other, trim, tall_cabinet) are no
    // longer fetched or exposed on `li` — only countertopItems (the
    // countertop material engine) remains, since it's the only pricing
    // category this widget still uses.
    const li = {
  ...
  // COUNTERTOP FORK: buildTRIM/TRIM (crown moulding & valance) and
  // buildTALLCAB/TALL_CAB (tall cabinets) — plus their trimOpts/tallCabOpts/
  // tallCabItems picker helpers — were entirely cabinet-specific and have
  // been removed along with the cabinet box-building flow.
  ...
  // same markup.
  // COUNTERTOP FORK: this previously described measuring cabinet runs
  // (upper/base cabinets, corner cabinets) — rewritten for countertop
  // surfaces. `roomId` is kept as a parameter for backward compatibility
  // with mqRefreshMeasureGuide's call site, but the guide text no longer
  // branches per room — countertop measuring guidance doesn't meaningfully
  // differ between Kitchen/Bathroom/etc. the way cabinet guidance did.
  function defaultMeasureGuideHTML(roomId = 'kitchen') {
    return `
      ...
      <div style="background:#fffbeb;border-radius:6px;padding:8px 10px;margin-bottom:10px;color:#92400e;font-size:12px">💡 <strong>All measurements will get converted with the ${mqCalcIconInlineHTML()} calculator.</strong> When you're ready, use the calculator to easily add in multiple sections and automatically convert inches/mm to feet.</div>
      <div style="margin-bottom:6px"><strong>Width:</strong> Measure the front edge of each countertop run, wall to wall (or wall to appliance/cabinet end), in inches.</div>
      <div style="margin-bottom:6px"><strong>Depth:</strong> Measure front-to-back — most standard counters run about 25.5", but islands, bars, and custom runs can differ.</div>
      <div style="margin-bottom:6px"><strong>Multiple runs or an island?</strong> Add each one as its own surface below — give it a name (e.g. "Island", "Perimeter", "Wet bar") so it's easy to tell apart on your quote.</div>
  ...
  // COUNTERTOP FORK: MQ_TERM_IMAGES/termHelpThumb (crown-return/valance-
  // return reference photos) were already unused dead code in the cabinet
  // widget and have been removed — they were cabinet-trim-specific anyway.
  ...
  function buildWidgetHTML(shop, specs, data) {
    ...
    return `
      ...
      <!-- COUNTERTOP FORK: the Both/Cabinets/Countertops tab bar and the
           three-tab layout it drove are gone — there is now exactly one
           flow. The project-type picker below (formerly cabinetForm's
           "Project basics" step) leads straight into the countertop
           configurator for every project type; no "Cabinets" or
           "Cabinets + Countertops" choice is ever presented. -->
      <div class="mq-tab-content active" id="mq-tab-countertops">
        ${priceLegendHTML()}
        <div class="mq-sec">
          ...
          <div class="mq-focal-box">
            ...
            <p class="mq-hint mq-focal-box-label" id="mq-ct-room-vanity-note" style="display:none;margin-top:8px"></p>
        ...
        <div class="mq-empty-calc-msg" id="mq-ct-empty-calc-msg" style="display:none">No selections have been made, or no measurements were entered — please double-check before calculating.</div>
        ...
        <div class="mq-result" id="mq-ct-result">
          <div class="mq-res-hdr">
            ...
            <div><div class="mq-res-range-lbl" id="mq-ct-res-range-lbl">Estimated range</div><div class="mq-res-range" id="mq-ct-res-range">—</div></div>
          </div>
  ...
  function wireWidget(data) {
    ...
    // COUNTERTOP FORK: window._mqCalcCabinet is no longer set (calcCabinet
    // was removed with the cabinet flow) — every generic prefix-aware
    // helper below (mqCommitCurrentConfig, mqCurrentLiveResult,
    // mqUpdateLivePreview, mqLiveRecalcSticky, _mqRefreshResultsPanel) has
    // been trimmed to its 'ct'-only branch accordingly.
    ...
    // COUNTERTOP FORK: P() built cabinet-only pricing config (box material/
    // door/drawer/hinge rates, cabinet install rates) purely for
    // calcCabinet — removed along with it. Its `removalRate` result was
    // also destructured (unused) inside calcCountertop; that dead
    // destructure has been removed there too.
    ...
    // COUNTERTOP FORK: diffOn (upper/base "different styles" split) and
    // tallCabs/tallCabCounts (tall cabinet cards) were cabinet-only state —
    // removed along with the cabinet flow and its tall-cabinet wizard.
    ...
    // COUNTERTOP FORK: window._mqActiveTabPrefix/mqSwitchTab (the Both/
    // Cabinets/Countertops tab switcher) and mqTogDiff (the "different
    // styles for uppers and lowers" cabinet toggle) are gone — there is
    // only one flow now, always 'ct', and no tab bar to switch between.
    window.mqTogVanityNote=(prefix)=>{
      // Always hidden from customers (kept as a no-op function, rather than
      // removing every call site, so nothing else breaks). Note: unlike the
      // original cabinet flow, calcCountertop does NOT apply the project
      // type's adjustment % to countertop pricing — see the report on this
      // fork for why that was left untouched rather than wired in.
  ...
  // COUNTERTOP FORK: added outdoor_kitchen/bar_island keys (both TODO
  // placeholders reusing the kitchen photo — see the default room types
  // above). laundry/garage/refacing/repainting/restaining are no longer in
  // the default room-type set but are left in place in case a shop still
  // has a legacy custom project type using one of those ids/names.
  const MQ_DEFAULT_COVER_IMAGES = {
    ...
    // TODO: placeholders — reuse the kitchen photo until real countertop-specific photos exist.
    outdoor_kitchen: 'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg',
    bar_island: 'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg',
  ...
  // COUNTERTOP FORK: the original default measure-guide photo set was a
  // cabinet-specific infographic series (how-to-measure, island, corner-
  // cabinets) that would be actively misleading for a countertop shop —
  // rather than ship the wrong photos, this now falls back to no images
  // (the text-only guide in defaultMeasureGuideHTML still shows). TODO: add
  // real countertop measuring photos here per project type once available.
  ...
  function mqDefaultImageKey(room) {
    ...
    if (name.includes('outdoor') || name.includes('bbq')) return 'outdoor_kitchen';
  ...
  window.mqShowRoomDescription=(prefix)=>{
    ...
    // COUNTERTOP FORK: this used to also do a large crown/valance/door-
    // linkage-specific pass (hiding "Pick a collection" groups and chips
    // that don't match the current door style, collapsing the whole trim
    // section if nothing qualifies) scoped to the Cabinets/Both tabs. That
    // entire pass only ever applied to cabinet trim pickers, which no
    // longer exist, so it's been removed — this now only does what it
    // always did for every tab/prefix (including the standalone
    // Countertops tab, which never had a room selector to trigger the
    // removed pass anyway): refresh the "more to scroll" arrows on every
    // picker row, since a row's chips can overflow regardless of prefix.
    ...
    // COUNTERTOP FORK: mqRefreshSectionVisibility (which hid/showed the
    // cabinet-measurements/drawers/tall-cabinets/trim sections and the Both
    // tab's cabinet-tied countertop options based on whether the current
    // project type had real box materials) was 100% cabinet-specific and
    // has been removed, along with its call sites.
    ...
    // Resets the countertop configurator back to its defaults so switching
    // project types always starts fresh. The `mq-${prefix}-cab-*` ids below
    // (cabinet-tied countertop fields) never exist in this fork's single
    // flow — those getElementById calls are harmless no-ops kept only
    // because they were already prefix-generic in the original file; the
    // parts that actually matter here are the supply/install reset and
    // clearing out any added surface cards.
    ...
    function mqSnapshotFormState(prefix) {
      ...
      // COUNTERTOP FORK: tall cabinet card snapshotting removed along with
      // the tall-cabinet wizard itself.
    ...
    function mqRestoreFormState(prefix, snapshot) {
      ...
      // COUNTERTOP FORK: tall cabinet card restoration removed along with
      // the tall-cabinet wizard itself.
    ...
    window.mqCommitCurrentConfig = function(prefix) {
      ...
      try {
        ...
        // calcCountertop and mqShouldShowRange both read the room dropdown's
        // CURRENT live value to decide which project type's range/no-range
        // setting applies — but by the time this runs (called from inside
        // the room dropdown's own onchange), the dropdown has already
        // switched to the NEW room. Without correcting for this, whatever's
        // being committed would silently pick up the NEW project type's
        // range setting instead of the one actually being left. Temporarily
        // rewind the dropdown to its previous value for the whole
        // calculation, then restore it to the actual new selection once done.
        ...
        try {
          // COUNTERTOP FORK: this used to branch on prefix === 'b' / 'ct' /
          // else (cabinets) — only the 'ct' (countertop) branch remains,
          // since 'ct' is the only prefix this fork ever uses.
          const r = window._mqCalcCountertop(prefix);
    ...
    // Jumps to the project-type picker (Project basics, step 1).
    // Reuses mqScrollWithOffset so a shop site's own sticky header doesn't
    // cover the landing spot.
    window.mqScrollToTop = function() {
      const target = document.getElementById('mq-ct-room') || document.getElementById('mq-tab-countertops') || document.getElementById('midasquote-widget');
    ...
    window.mqOnProjectTypeChange = function(prefix) {
      ...
      // COUNTERTOP FORK: mqResetCabinetForm(prefix) → mqResetCountertopStandalone(prefix).
      // Unlike the cabinet form (which re-seeds a starter tall-cabinet card
      // itself), the countertop reset just empties the surfaces container —
      // so when this is a genuinely new (not cart-restored) project type,
      // add back one blank starter surface, matching the very first page
      // load's behavior (see the end of wireWidget).
      mqResetCountertopStandalone(prefix);
      if (!restoreSnapshot) addSurfaceInternal(prefix);
    ...
    // COUNTERTOP FORK: mqTogDwOption (dishwasher gap, Both-tab only),
    // mqTogTrimReturns/mqTogTrimManualFt/mqTogTrimUseCab/mqMarkSuggestedChip/
    // mqApplyLinkedTrim (crown/valance ↔ door-style linkage), and
    // mqTogDrawerConfig (drawer tier picker) were all cabinet-only UI glue
    ...
    // COUNTERTOP FORK: the tall-cabinet wizard (renumberTallCabs,
    // addTallCabInternal, mqAddTallCab, mqValidateTallCabWidth,
    // mqRemoveTallCab, mqAdjTallCabQty, mqTogTallCabNone) was entirely
    // cabinet-specific and has been removed.
    ...
    function calcCountertop(prefix) {
      const ctSiId='mq-ct-si';
      const lines=[]; let sub=0;

      // Project type is used for labeling (roomLabel, below) and for the
      // specialty-items room-visibility filtering that already runs
      // elsewhere — deliberately NOT applied as a price multiplier here.
      // The countertop material engine (CT_MAT, rates, minimums) must stay
      // exactly as it was in the original cabinets+countertops widget, so
      // room-based % adjustments (materialAdjPct/installAdjPct/totalAdjPct)
      // are intentionally not read or applied in this function.
      const roomId = gv(`mq-${prefix}-room`);
      const roomObj = (window._mqRoomTypes||[]).find(r=>r.id===roomId);
      const roomLabel = roomObj ? roomObj.name : 'Countertops';
      ...
      // COUNTERTOP FORK: the "measure via cabinet run" block (tying a
      // countertop's linear footage to the cabinet box measurements on the
      // combined Both tab, via an `mq-${prefix}-use-cab` checkbox) was
      // removed — that checkbox and its whole cabinet-run sub-form
      // (mq-${prefix}-cab-mat/-bs/-co/-cuts/-dw/-edge/-addons) never existed
      // in the standalone countertop flow's HTML to begin with, so this was
      // already 100% dead code (window.mqTogUseCab, its only trigger, is
      // removed too). Every counter is now priced only via the surface
      // cards below, same as the original standalone Countertops tab.
      ...
      // Specialty items — ported unchanged from the original calcCabinet so
      // "Details & Selections" (specHTML) pricing actually applies to the
      // standalone countertop flow now that it's rendered here too.
      let specTotal=0;
      specs.forEach((s,i)=>{
        if(!specQty[prefix] || !specQty[prefix][i]) return;
        ...
        const installQtyVal = (installKind !== supplyKind) ? ((installQty[prefix] && installQty[prefix][i]) || 0) : supplyQty;
      ...
      const total=sub;
      return {lines,sub:Math.round(sub),total:Math.round(total),low:Math.round(total*(window._mqRangeLow||0.95)/10)*10,high:Math.round(total*(window._mqRangeHigh||1.20)/10)*10,roomLabel};
    }
    ...
    window._mqRefreshResultsPanel = function(prefix) {
      if (prefix === 'ct') {
        ...
        return { low: r.low, high: r.high, total: r.total, label: r.roomLabel || 'Countertops' };
      }
      return null;
    };

    // COUNTERTOP FORK: window.mqCalcCabinets and window.mqCalcBoth (the
    // Calculate handlers for the 'c' and 'b' flows) were removed along with
    // calcCabinet and the rest of the cabinet-building UI — this is now the
    // only Calculate handler.
    window.mqCalcCountertops=()=>{
      ...
      window.mqShowLead(async lead=>{
        ...
        setTimeout(async()=>{
          ...
          if(lead) await mqSaveLeadWithCart(data,lead,'Countertops',r.low,r.high,r.lines,r.roomLabel||'',r.total,'ct');
        },900);
      });
    };
    ...
    // COUNTERTOP FORK: window.mqTogUseCab (toggled the "measure via cabinet
    // run" checkbox's sub-form) was never referenced by the standalone
    // countertop flow's HTML — removed along with that dead code in
    // calcCountertop above.
    ...
    // COUNTERTOP FORK: window.mqTogCabCuts and window.mqTogCabExtra (toggles
    // inside the "measure via cabinet run" sub-form) were removed along
    // with the rest of that dead sub-form — see the note in calcCountertop.
    ...
    // COUNTERTOP FORK: window.mqRefreshBsFt (backsplash-footage readout for
    // the "measure via cabinet run" sub-form, keyed off cabinet linear feet)
    // was removed along with the rest of that dead sub-form. The standalone
    // surface cards' own backsplash readout (window.mqRefreshSurfBsFt,
    // right below) is unaffected and still fully wired.
    window.mqRefreshSurfBsFt=(id)=>{
    ...
    // COUNTERTOP FORK: window.mqSyncCtSi (synced the "Both" tab's separate
    // countertop supply/install field to match the cabinet choice) was
    // cabinet+both-only and unreferenced by anything in the countertop-only
    // flow — removed.
    ...
    function mqValidateNotEmpty(prefix, result) {
      ...
      // COUNTERTOP FORK: the original also distinguished "picked doors/
      // crown/etc but never entered linear feet" for the cabinet flow —
      // that branch (and its diffOn[prefix] lookup) was cabinet-only and
      // was removed along with the cabinet UI. The 'no surfaces added at
      // all' case is already caught earlier by mqCalcCountertops itself, so
      // by the time we get here it's always the generic empty-quote message.
      ...
      if (msgEl) {
        msgEl.textContent = "No selections have been made, or no measurements were entered — please double-check before calculating.";
    ...
    addSurfaceInternal('ct');
    // COUNTERTOP FORK: the tall-cabinet auto-add block (addTallCabInternal)
    // and the mqRefreshSectionVisibility calls below were cabinet-only and
    // were removed along with that code. Room-visibility filtering still
    // applies right away for whatever room is selected by default —
    // specialty items render unfiltered in HTML first, then get filtered
    // here so we don't need to know the room at HTML-build time.
  ...
  // by calling the same pure calcCountertop function Calculate itself uses
  // — no popups, no re-saving a lead, just silent re-math.
  window._mqStickyPrefix = null;
  window._mqStickyDismissed = false;
  window._mqStickyLast = null;
  ...
  function mqCurrentLiveResult() {
    ...
    const r = window._mqCalcCountertop(prefix);
    return { prefix, low: r.low, high: r.high, total: r.total, lines: r.lines, quoteType: 'Countertops', roomLabel: r.roomLabel || '' };
  }
  ...
  // Countertops is now tied to a real project-type selection (mq-ct-room),
  // so it respects that room's own showRange setting the same way the old
  // Cabinets/Both flows did (defaulting to true/range for any room that's
  // never touched this).
  ...
  function mqUpdateLivePreview(prefix, precomputedRange) {
    ...
    if (!range) {
      ...
      const r = window._mqCalcCountertop(prefix);
      range = { low: r.low, high: r.high, total: r.total, label: r.roomLabel || 'Countertops' };
    }
