/*
 * MidasQuote PRO QUOTER — COUNTERTOP STORE EDITION (forked from widgetpro.js)
 ...
 *
 * COUNTERTOP-ONLY FORK: this build serves MidasQuote's countertop-only
 * store product line. Every cabinet box-building feature (materials,
 * doors, drawers, hinges, tall cabinets, crown/valance trim, and the
 * "Cabinets" / "Cabinets + Countertops" tabs) has been removed — the
 * Countertop configurator (surfaces, countertop materials, edge profiles,
 * backsplash, cutouts, addons) is now the widget's only flow, entered
 * straight from the project-type picker. Specialty Items and every
 * Pro-only real-price feature are kept fully intact. Keep this in sync
 * with widget.js's countertop fork (widget-countertop.js) — see
 * persistent-fixes-checklist.md for why the two builds drifting apart has
...
(function() {
  ...
  async function loadShopData(token) {
    ...
    // price adjustment %. Falls back to a fixed set of countertop-relevant
    // project types (with Bathroom's -5% preserved as a working example)
    // for every shop that hasn't touched this new setting yet, so nothing
    // changes for anyone until they actively configure it.
    //
    // COUNTERTOP FORK: replaced the cabinet-store default list (which
    // included Laundry room / Garage / Refacing / Repainting / Restaining —
    // all cabinet-refinishing project types with no countertop equivalent)
    // with a countertop-store-relevant list: Kitchen, Bathroom, Outdoor
    // Kitchen / BBQ, Bar / Island, Commercial, Other. Kitchen/Bathroom keep
    // their original photos; the three new types below reuse the closest
    // existing default image as a placeholder — TODO(human): replace
    // coverImage/measureImage for 'outdoor', 'bar', and 'commercial' with
    // real countertop-specific photos once available.
    let roomTypes = [];
    try { roomTypes = shop['Room types'] ? JSON.parse(shop['Room types']) : []; } catch(e) { roomTypes = []; }
    if (!Array.isArray(roomTypes) || !roomTypes.length) {
      roomTypes = [
        { id:'kitchen', name:'Kitchen',        adjustment:0,  description:'The kitchen is where life happens — let\'s help you find the perfect countertop to bring it all together.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/kitchen1.jpg' },
        { id:'bathroom',name:'Bathroom',       adjustment:-5, description:'Turn your bathroom into a personal retreat. Choose the vanity top and finishes that make getting ready each morning feel a little more special.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/bathroom.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/bathroom1.jpg' },
        // PLACEHOLDER IMAGES — reusing the closest existing default photo
        // until real Outdoor Kitchen / BBQ photography is available.
        { id:'outdoor', name:'Outdoor Kitchen / BBQ', adjustment:0,  description:'Built to handle the weather without giving up on style — let\'s find the right surface for your outdoor kitchen or BBQ island.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/kitchen1.jpg' },
        // PLACEHOLDER IMAGES — reusing the closest existing default photo
        // until real Bar / Island photography is available.
        { id:'bar',     name:'Bar / Island',   adjustment:0,  description:'A bar or island top gets noticed — let\'s pick a material and edge that makes it the centerpiece of the room.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/kitchen1.jpg' },
        { id:'commercial', name:'Commercial',  adjustment:0,  description:'Make a great first impression. Get countertops built to fit your business, whether it\'s a sleek office or a welcoming retail space.', active:true, coverImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/commercial.jpg', measureImage:'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/measure-guides/commercial1.jpg' },
    ...
    // COUNTERTOP FORK: dropped the cabinet-only line-item categories here —
    // materials/rawMaterials ('material'), doorStyles ('door'), drawers
    // ('drawer'), hinges ('hinge'), trimItems ('trim' — crown/valance),
    // tallCabItems ('tall_cabinet') — along with their photo/visibleRooms
    // enrichment loops and the `hasDynamic` (has-real-materials) flag they
    // fed. installItems/taxItems/otherItems are kept even though they were
    // only ever read by the removed cabinet pricing engine (P()/
    // calcCabinet), in case a future shared feature needs shop-level
    // install/tax/removal rates again.
    const li = {
    ...
    // Pro sees every specialty item, including ones flagged "Pro only" —
    // that flag only ever hides an item from the plain widget.js, never
    // from here — so unlike widget.js there is deliberately no filter on
    // specRecords before mapping.
    ...
    return { shop, pricing:p, specs, li, shopPhotos, shopFeatured, roomTypes };
  }
  ...
  async function saveLead(data, lead, quoteType, low, high, lines, realTotal, prefix) {
    const { shop } = data;
    ...
    await sendEmail(lead.email, `${quoteType} quote — ${shop['Shop name']}`,
      `<div style="font-family:sans-serif;max-width:560px;margin:0 auto">
        ...
        <div style="background:#0f2a52;border-radius:8px;padding:16px;text-align:center;margin-bottom:12px">
          ...
          <div style="font-size:28px;font-weight:700;color:#fff">${CUR()}${Math.round(realTotal||0).toLocaleString()}</div>
        </div>
  ...
  function injectStyles(bc) {
    ...
    s.textContent = `
      ...
      /* The round scroll arrows every other picker uses are absolutely
         ...
         top of SOME pill's text in this slimmer picker. Real fix for
         variant pickers specifically: the arrows are genuine flex siblings
         of the scroll row (see mqVariantScrollWrap), not absolutely
         positioned over it — each has its own reserved column of space to
         the left/right of the pills, so there's no scroll position where
         either arrow can ever sit on top of pill text. Reuses the exact
         same ids/classes mqUpdatePickerArrow/mqScrollPickerRow already
         toggle and click, so no JS changes were needed, only where the
         buttons render. */
  ...
  // COUNTERTOP FORK: removed buildTRIM/trimOpts (crown & valance moulding —
  // 'trim' / trim_crown / trim_valance categories) and buildTALLCAB/
  // tallCabOpts/tallCabItems (the 'tall_cabinet' category and its wizard) —
  // both purely cabinet-box features with no countertop equivalent.
  ...
  function specHTML(specs, prefix) {
    ...
    const buildCard = (s,i,groupKey,groupIndex) => {
      ...
      // The actual supply/install CHOICE control sits under the photo,
      // left-aligned to match the image's own width, above the variant
      // pills — its own visually distinct row rather than crammed into the
      // description column, so the image/dropdown/pills all line up
      // cleanly down the left side of the card. An item that only ever has
      // ONE install mode (nothing to choose) keeps its plain-text label
      // inline with the description instead.
  ...
  // same markup.
  // COUNTERTOP FORK: rewritten from the ground up — the original copy here
  // described measuring upper/base/island CABINET runs (and illustrated a
  // "corner cabinets" overlap trick with a cabinet-labeled photo), none of
  // which applies to a countertop-only flow. This version describes
  // measuring countertop sections instead: each one's front-edge length and
  // depth feed straight into that surface card's own Width/Depth fields
  // (see addSurfaceInternal) rather than a single shared linear-ft total.
  // STILL NEEDS HUMAN REVIEW: the corner-run illustration below has no
  // photo at all (the original's image was explicitly labeled "corner
  // cabinets" and would have been wrong to reuse relabeled) — a real
  // corner-countertop measuring photo should be sourced and added here,
  // the same way the fallback project-type cover images below are flagged
  // as placeholders needing real photos.
  function defaultMeasureGuideHTML(roomId = 'kitchen') {
    const cornerNote = `<div style="margin-bottom:6px"><strong>Corner or L/U-shaped counters:</strong> Add a separate surface below (the "+ Add another surface" button) for each wall run rather than trying to combine them into one width. Where two runs meet at a corner, it's fine to measure each run to roughly where the corner falls — don't worry about the exact number, this is a ballpark estimate.</div>`;
    return `
      <div style="font-weight:600;margin-bottom:18px;color:#111">📏 Quick measuring guide</div>
      <div style="background:#fffbeb;border-radius:6px;padding:8px 10px;margin-bottom:10px;color:#92400e;font-size:12px">💡 <strong>Width and depth measurements will get converted with the ${mqCalcIconInlineHTML()} calculator.</strong> When you're ready, use the calculator to easily add in multiple sections and automatically convert inches/mm to feet.</div>
      <div style="margin-bottom:6px"><strong>Width:</strong> Measure the front-edge length of the counter as it will be cut — this is the "Width" field on each surface below.</div>
      <div style="margin-bottom:6px"><strong>Depth:</strong> Front-to-back distance from the wall to the counter's front edge. A standard depth is already filled in, but override it for anything deeper or shallower (e.g. an island).</div>
      <div style="margin-bottom:6px"><strong>Islands & peninsulas:</strong> Give each one its own surface — click "+ Add another surface" rather than folding it into another run's width.</div>
      ${roomId !== 'bathroom' ? cornerNote : ''}
  ...
  function mqEnsureCalcModal() {
    ...
    // same fix as the proposal modals — or the sticky bar renders on top
    // of and covers the bottom of this modal.
  ...
  function mqRenderCalc() {
    ...
    card.innerHTML = `
      ...
      ${_mqCalcMode !== 'inches' ? `<button type="button" onclick="mqCalcAddSection()" style="width:100%;padding:8px;border-radius:6px;border:1.5px dashed #d97706;background:#0f2a52;color:#fbbf24;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;margin-bottom:14px">+ Add another section</button>` : ''}
  ...
  function collapsibleHeader(key, title, startOpen) {
    ...
    // COUNTERTOP FORK: the inline mqJumpToSectionIfNeeded(...) call below is
    // now a no-op stub — it drove the guided-step-flow's "jump to this
    // section" focus tracking, which is removed entirely (see its
    // definition further down for why it's kept as a stub rather than
    // deleted: this template still calls it on every header click).
  ...
  // COUNTERTOP FORK: removed cabinetForm() entirely — the cabinet box-
  // building form (materials, doors, drawers, hinges, tall cabinets,
  // crown/valance trim). Its generic, reusable pieces (the "Project
  // basics" project-type picker and the "Details & Selections"
  // specialty-items block) were ported directly into buildWidgetHTML's
  // single countertop content block below, with prefix 'ct'.
  ...
  function buildWidgetHTML(shop, specs, data) {
    // COUNTERTOP FORK: removed the unused `hasCtInstall` (was only read by
    // the "Both" tab's now-gone cabinet-attached countertop section — the
    // standalone Countertop UI kept below never used it, each surface's own
    // supply/install dropdown already calls hasCountertopInstall() itself
    // inside addSurfaceInternal) and `disc` (was only read by the removed
    // Cabinet/Both tabs' disclaimer text — the Countertop result panel below
    // has always used its own static "Stone slabs vary by lot..." wording).
    ...
    return `
      ...
      <!--
        COUNTERTOP FORK: the "Both / Cabinets / Countertops" tab bar and its
        three tab-content blocks (CABINET TAB, COUNTERTOP TAB, BOTH TAB) are
        gone — that branching only ever existed to let a customer choose
        between a cabinet quote, a countertop quote, or a combined one.
        This build has exactly one flow: project type, then straight into
        the countertop configurator (surfaces + specialty items) below —
        the same markup the old standalone Countertop tab used, with the
        cabinetForm()-sourced "Project basics" project-type picker and
        "Details & Selections" specialty-items block (both fully generic,
        not cabinet-specific) added ahead of it so specialty items keep a
        working home now that the cabinet tabs that used to render them
        are gone.
      -->
      <div class="mq-tab-content active" id="mq-ct-content">
        ${priceLegendHTML()}
        <div class="mq-sec">
          ...
          <div style="background:#F0E9DA;border:2px solid #0f2a52;border-radius:12px;padding:16px 18px">
            ...
            <select id="mq-ct-room" onfocus="window._mqPrevRoomId=window._mqPrevRoomId||{};window._mqPrevRoomId['ct']=this.value" onchange="mqCommitCurrentConfig('ct');mqRefreshRoomVisibility('ct');mqShowRoomDescription('ct');mqRefreshMeasureGuide('ct');mqRefreshAllPickerVisibility('ct');mqOnProjectTypeChange('ct')" style="font-size:15px;font-weight:600;padding:10px 12px">${(data.roomTypes||[]).filter(r=>!r.hideFromPro).map(r=>`<option value="${r.id}">${r.name}</option>`).join('')}</select>
            <p style="display:block;margin-top:8px;font-weight:500;color:#0f2a52;font-size:13px">${shop['Project type hint'] || 'After calculating your first quote, you can continue adding other project types.'}</p>
            <div id="mq-ct-room-desc" style="display:none;margin-top:8px;padding:10px 12px;background:#fff;border:1px solid #0f2a52;border-radius:6px;font-size:13px;color:#1f2937;line-height:1.5"></div>
          </div>
        </div>
        ...
        <div class="mq-sec" id="mq-ct-specialty-sec" onclick="mqOpenIfClosed('ct-specialty')">
          ${collapsibleHeader('ct-specialty', 'Details & Selections')}
          <div style="font-size:13px;color:#4b5563;margin-bottom:10px;line-height:1.5">
          ...
          <div id="mq-ct-specialty-body" style="display:none">
            <div class="mq-spec-grid">${specHTML(specs, 'ct')}</div>
          </div>
        </div>
  ...
  function wireWidget(data) {
    const { shop, pricing, specs, li, shopPhotos } = data;
    ...
    // the exact same pure calculation function Calculate itself uses, for
    // live updates as the customer tweaks anything. Safe to reference here
    // even though calcCountertop is defined further down in this function's
    // body — function declarations are fully hoisted within their enclosing
    // scope, so it's already callable by this point.
    window._mqCalcCountertop = calcCountertop;

    // Seed the "room being left" tracker from the room dropdown's actual
    ...
    {
      const roomEl = document.getElementById('mq-ct-room');
      if (roomEl) window._mqPrevRoomId['ct'] = roomEl.value;
    }

    // COUNTERTOP FORK: removed P() entirely — it built box-material/door/
    // drawer/hinge rate tables plus cabinet install & removal rates, none of
    // which the countertop engine below ever reads (calcCountertop only
    // destructured `removalRate` from it and never actually used the
    // value — see the matching removal of that destructure further down).
    // Also removed the drawerConfigNames helper it fed (built from the
    // now-gone li.drawers).
    ...
    const specQty={},installQty={},specVariant={},surfCounts={},surfs={};
    ...
    let pendingCb=null;
    // COUNTERTOP FORK: only 'ct' remains (was ['c','ct','b']). Also dropped
    // diffOn/tallCabs/tallCabCounts from this whole tracking-state block —
    // diffOn backed the cabinet "different styles for uppers and lowers"
    // toggle (mqTogDiff, removed) and tallCabs/tallCabCounts backed the
    // tall-cabinet card system (also removed); nothing reads either anymore.
    ['ct'].forEach(p=>{specQty[p]=new Array(specs.length).fill(0);installQty[p]=new Array(specs.length).fill(0);specVariant[p]=new Array(specs.length).fill(0);surfCounts[p]=0;surfs[p]={};});
    ...
    // COUNTERTOP FORK: removed mqSwitchTab (the Both/Cabinets/Countertops
    // tab switcher — there's only one flow now, so nothing to switch
    // between) and mqTogVanityNote (an already-no-op stub for a
    // cabinet-only "room adjustment" note whose markup no longer exists in
    // this build).
  ...
  // sensibly instead of just showing nothing. Only the default countertop
  // project types below have real fallback assets; anything else (a shop's
  // own custom-named project type) has no fallback and simply shows blank,
  // same as before.
  //
  // COUNTERTOP FORK: keys updated to match the new default project-type
  // list (kitchen/bathroom/outdoor/bar/commercial/other, replacing the
  // cabinet-store set's laundry/garage/refacing/repainting/restaining).
  // 'outdoor' and 'bar' reuse the kitchen image as a placeholder — see the
  // TODO(human) note on the default roomTypes array above.
  const MQ_DEFAULT_COVER_IMAGES = {
    ...
    // PLACEHOLDERS — reusing the kitchen photo until real photography exists.
    outdoor: 'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg',
    bar: 'https://raw.githubusercontent.com/aceswin/midasquote-widget/main/cover-images/kitchen.jpg',
  ...
  // STILL NEEDS HUMAN REVIEW: 'corner-cabinets.jpg' in this carousel set is
  // (per its own filename) a cabinet-specific photo carried over unchanged
  // from the original shared set — worth checking whether it actually shows
  // something countertop-relevant (a corner counter seam, say) or should be
  // swapped/dropped for this fork, same concern as the placeholder cover
  // images and defaultMeasureGuideHTML's corner-run copy above.
  ...
  const MQ_DEFAULT_MEASURE_IMAGES = {
    ...
    // PLACEHOLDERS — reusing the generic measure-guide set until real
    // countertop-specific measuring photos exist for these project types.
    outdoor: MQ_DEFAULT_MEASURE_IMAGE_SET,
    bar: MQ_DEFAULT_MEASURE_IMAGE_SET,
    commercial: MQ_DEFAULT_MEASURE_IMAGE_SET,
    other: MQ_DEFAULT_MEASURE_IMAGE_SET,
  };

  // Matches a room to one of the default-image keys above. Tries the id
  ...
  // being "Bar / Island" or "Commercial" by name.
  function mqDefaultImageKey(room) {
    ...
    if (name.includes('outdoor') || name.includes('bbq')) return 'outdoor';
    if (name.includes('bar') || name.includes('island')) return 'bar';
  ...
  window.mqShowRoomDescription=(prefix)=>{
    ...
    // COUNTERTOP FORK: simplified from the original 'c'/'b' version. Removed
    // entirely: the crown/valance-only first pass that hid "Pick a
    // collection" options with zero door-eligible members (no crown/valance
    // exists here), and the isTrimRow-specific door-linkage/wrap-visibility
    // logic in the main pass — none of that applies to countertop material
    // or specialty-item picker rows, the only grouped pickers left. What's
    // kept is exactly the same room+group chip-visibility logic the
    // original applied to every OTHER (non-trim) row, generalized to run
    // for prefix 'ct' over the whole widget instead of one tab's scope
    // (there's only one content area now, so no separate scope needed).
    window.mqRefreshAllPickerVisibility=(prefix)=>{
      if (prefix === 'ct') {
        const roomId = gv('mq-ct-room');
        const scope = document.getElementById('midasquote-widget');
        if (scope) {
    ...
    function mqScrollPoweredByAboveSticky(prefix) {
      const resultId = `mq-${prefix}-result`; // COUNTERTOP FORK: only 'ct' is ever passed in now
      let attempts = 0;
      function tryScroll() {
        attempts++;
    ...
    // COUNTERTOP FORK: the entire "guided step flow" subsystem that used to
    // live here (mqGetVisibleSections, mqRenumberSteps, _mqStepIndex,
    // mqEnsureStepFooter, mqUpdateStepFocus, mqHighlightCalcButton,
    // mqStepContinue/Back, the click-driven mqJumpToSectionIfNeeded, and its
    // IntersectionObserver-based scroll-spy) was removed outright: in the
    // original file it was already gated to prefix 'c'/'b' only
    // (mqGetVisibleSections returned [] for any other prefix, including
    // 'ct'), so it was 100% dead code for the countertop-only flow even
    // before this fork — nothing here is a behavior change for 'ct'.
    // mqJumpToSectionIfNeeded is kept as a no-op stub immediately below
    // because collapsibleHeader()'s onclick template still calls it inline
    // on every section header click.
    window.mqJumpToSectionIfNeeded = function(_sec) {};

    // COUNTERTOP FORK: mqRefreshSectionVisibility (cabinet-measurements/
    // drawers/tall-cabinets/trim/crown-valance section show-hide logic,
    // gated to prefix 'c'/'b' in the original) removed entirely — every
    // call site was inside cabinet-only code that's also been deleted
    // (mqResetCabinetForm, addTallCabInternal, the guided-step-flow block).
    ...
    // COUNTERTOP FORK: mqResetCabinetForm (reset for box material/door/
    // hinge/drawer/tall-cabinet/trim fields, plus the 'b'-only countertop
    // sub-block) removed entirely — mqResetCountertopStandalone below is
    // now the only reset path 'ct' needs.

    // Resets the countertop-only form: the "use cabinet measurements" path
    // (uft/bft, now vestigial since box cabinets no longer exist here but
    // still wired to the "use my measurements" checkbox) plus any added
    // surfaces (islands, peninsulas, etc.).
    ...
    function mqSnapshotFormState(prefix) {
      ...
      // COUNTERTOP FORK: dropped the tall-cabinet snapshot block that used
      // to live here — tallCabs/tallCabCounts and the whole tall-cabinet
      // card system are cabinet-only and removed from this fork.
    ...
    function mqRestoreFormState(prefix, snapshot) {
      if (!snapshot) return;
      // COUNTERTOP FORK: dropped the diffOn/mqTogDiff restore that used to
      // run first here — mqTogDiff (the upper/base cabinet split toggle) was
      // cabinet-only and has been removed; diffOn[prefix] is always false
      // for 'ct', so this branch would never have fired anyway, but it was
      // a dangling call to a since-deleted function and worth removing
      // outright.
      ...
      // COUNTERTOP FORK: dropped the tall-cabinet-card restore block that
      // used to run here — tall cabinets are cabinet-only and removed.
    ...
    window.mqCommitCurrentConfig = function(prefix) {
      // COUNTERTOP FORK: originally only 'b'/'c' had a room dropdown to read
      // here — 'ct' was standalone-only in the old three-tab layout, with no
      // room selector of its own, so it was excluded. This fork's merged
      // single-flow layout gives 'ct' a real `mq-ct-room` element, so it's
      // now included too (this only matters if mqCommitCurrentConfig is ever
      // called for 'ct' — see mqOnProjectTypeChange below).
      const roomEl = document.getElementById(`mq-${prefix}-room`);
      const actualValue = roomEl ? roomEl.value : null;

      try {
        if (!window._mqCalcCountertop) return false;
    ...
    window.mqRenderQuoteCart = function() {
      ...
      const realBadge = entry => entry.showRange
        ? `<span style="font-size:11.5px;font-weight:700;color:#fbbf24;white-space:nowrap">💰 Real: ${CUR()}${Math.round(entry.total).toLocaleString()}</span>`
        : '';
    ...
    // COUNTERTOP FORK: originally picked between three tabs' worth of
    // possible landing spots (Cabinets/Both/Countertops); there's only one
    // flow now, so this always jumps to the project-type picker.
    // Reuses mqScrollWithOffset so a shop site's own sticky header doesn't
    // cover the landing spot.
    window.mqScrollToTop = function() {
      const target = document.getElementById('mq-ct-room') || document.getElementById('mq-ct-content') || document.getElementById('midasquote-widget');
      mqScrollWithOffset(target, 80);
    };

    // Clears the whole running quote and resets the form back to its
    // starting state, so the customer can start completely over.
    ...
    window.mqOnProjectTypeChange = function(prefix) {
      ...
      // COUNTERTOP FORK: dropped `_mqStepIndex[prefix] = 0;` — that was the
      // guided-step-flow's own per-tab step counter, and the whole guided
      // step flow subsystem is removed (it was already dead code for 'ct').
      ...
      // COUNTERTOP FORK: mqResetCabinetForm(prefix) replaced with
      // mqResetCountertopStandalone(prefix) — the only form left to reset.
      // Also re-seeds one starter countertop surface when there's no cart
      // snapshot being restored for this project type, since
      // mqResetCountertopStandalone clears #mq-${prefix}-ct-surfaces without
      // re-adding one (the original three-tab layout relied on
      // mqRefreshSectionVisibility's 'b'-only auto-add for that; this
      // single-flow layout has no such fallback, so it's done explicitly
      // here instead).
      mqResetCountertopStandalone(prefix);
      if (specQty[prefix]) {
      ...
      // COUNTERTOP FORK: dropped the mqRefreshSectionVisibility(prefix) call
      // — that function (cabinet-measurements/drawers/tall-cabs/trim
      // section show-hide logic) is removed entirely.
      if (restoreSnapshot) {
        mqRestoreFormState(prefix, restoreSnapshot);
      } else {
        // No cart entry to restore for this project type — reseed one
        // starter countertop surface, same as first page load, so the
        // section isn't left empty with nothing but an "+ Add another
        // surface" button. mqResetCountertopStandalone (above) cleared the
        // surfaces container and surfs[prefix] to get to a blank slate.
        addSurfaceInternal(prefix);
      }
      mqRefreshBallparkWording(prefix);
      if (window._mqStickyPrefix === prefix) mqUpdateLivePreview(prefix);
    };
    // COUNTERTOP FORK: removed the whole cabinet-only toggle cluster that
    // used to live here — mqTogDwOption (dishwasher-cutout toggle),
    // mqTogTrimReturns / mqTogTrimManualFt / mqTogTrimUseCab (crown/valance
    // returns & manual-footage toggles), mqMarkSuggestedChip and
    // mqApplyLinkedTrim (door-to-crown/valance style linking), and
    // mqTogDrawerConfig (drawer-tier config visibility). None of these apply
    // without box materials, doors, drawers, or trim.
    ...
    // Applying a variant pick just mutates the `specs[i]` object this card
    ...
    // COUNTERTOP FORK: removed the entire tall-cabinet card system that used
    // to live here — renumberTallCabs, addTallCabInternal, mqAddTallCab,
    // mqValidateTallCabWidth, mqRemoveTallCab, mqAdjTallCabQty, and
    // mqTogTallCabNone. Tall cabinets are box-cabinet-only.
    ...
    // COUNTERTOP FORK: removed getMaterialRates() and calcCabinet() entirely
    // — the whole cabinet box/tall-cabinet/crown-valance pricing engine.
    // calcCountertop() below now also computes specialty-item pricing
    // (previously only calcCabinet did this — see the "Specialty items"
    // block inside calcCountertop for the ported logic, unchanged from
    // calcCabinet's version) so Specialty Items keep working now that the
    // cabinet flow that used to host them is gone.

    function calcCountertop(prefix) {
      // COUNTERTOP FORK: removed the `prefix === 'b'` early-return (the
      // "Both" tab's countertop-details-hidden guard — there's no cabinet
      // flow left for a countertop section to attach to or hide alongside)
      // and the `const {removalRate}=P();` destructure — P() itself is
      // removed, and `removalRate` was already unused here in the original
      // file (destructured but never referenced below). Also removed the
      // whole `useCabMeasure` "cabinet run" block that used to let the
      // "Both" tab reuse its base-cabinet linear footage as a countertop
      // run (mq-${prefix}-use-cab, -bft, -ct-mat-cab, -cab-bs/-co/-cuts/
      // -dw/-extra-ft) — none of those fields exist in this build, so the
      // block was already fully dead for the standalone Countertop tab
      // this fork is built from (its `useCabMeasure` checkbox was never
      // rendered outside the "Both" tab).
      const ctSiId = 'mq-ct-si'; // no project-level SI field in this build — see surfaces loop below, "inherit" is never offered per-surface
      const lines=[]; let sub=0;

      // Minimum charges pool PER MATERIAL, across every counter/run in this
      // one project type (every surface in the loop below) — not per
      ...
      // Specialty items — COUNTERTOP FORK: ported verbatim from the removed
      // calcCabinet() (the only place that priced Specialty Items before
      // this fork). Unchanged pricing logic: flat/per-linear-ft/per-sqft
      // rate, a supply-side minimum-price floor, an independent
      // install-side price/minimum when the item offers a separate install
      // choice, and the currently-picked variant's label folded into the
      // line text. Without this block, Specialty Items would render (the
      // UI is generic and prefix-based) but price as $0 — this is what
      // keeps the Specialty Items engine actually working now that the
      // cabinet flow that used to host its pricing is gone.
      let specTotal=0;
      specs.forEach((s,i)=>{
    ...
    window._mqRefreshResultsPanel = function(prefix) {
      if (prefix !== 'ct') return null; // COUNTERTOP FORK: 'c'/'b' branches removed along with calcCabinet
    ...
    // COUNTERTOP FORK: removed mqCalcCabinets and mqCalcBoth (the 'c'/'b'
    // Calculate handlers) along with the cabinet flow they belonged to.
    // mqCalcCountertops below now also runs mqValidateInstallQty — Specialty
    // Items' "install quantity still needs entering" check — since in the
    // original file that check only ever ran from the removed cabinet-tab
    // Calculate handlers (Specialty Items had no UI on the standalone
    // Countertop tab to validate); it's a no-op if nothing needs it.
    window.mqCalcCountertops=()=>{
      ...
      if (!mqValidateInstallQty('ct')) return;
    ...
    // COUNTERTOP FORK: removed mqSyncCtSi — it only ever did anything for
    // the "Both" tab's separate cabinet-si/countertop-si sync, was already
    // unused (no call site anywhere in the original file with prefix 'ct'),
    // and 'b' no longer exists.
    ...
    // COUNTERTOP FORK: simplified — the original also distinguished "picked
    // a door/crown/valance style but forgot to enter linear feet" for a
    // more specific nudge; none of those cabinet fields exist in this
    // build (they read as empty/false, so that branch could never fire
    // here), so this now just blocks Calculate with the generic message
    // when nothing priced out to anything.
    function mqValidateNotEmpty(prefix, result) {
      ...
      if (msgEl) {
        msgEl.textContent = "No selections have been made, or no linear feet was entered — please double-check before calculating.";
    ...
    // COUNTERTOP FORK: dropped the tall-cabinet auto-add block that used to
    // run here (TALL_CAB/addTallCabInternal are both removed) and collapsed
    // the 'c'/'b' room-visibility-filtering tail down to 'ct' only —
    // mqRefreshSectionVisibility is also removed (cabinet-only section
    // show/hide), so it's dropped from this sequence entirely.
  ...
  window.mqStartNewEstimate = function() {
    ...
    buildCTMAT(data);
    // COUNTERTOP FORK: dropped buildTRIM(data)/buildTALLCAB(data) — both
    // functions (and the TRIM/TALL_CAB tables they built) are removed.
  ...
  // COUNTERTOP FORK: the original comment here described this as a backstop
  // for the guided-flow scroll-spy (mqObserveSectionsForScrollSpy, inside
  // wireWidget), which opened each section as it crossed the exact vertical
  // center of the screen while scrolling. That scroll-spy — and the rest of
  // the guided step flow it belonged to — is removed entirely in this fork
  // (it was already dead code for 'ct'). This function is independent of
  // it and still does real, needed work on its own: a collapsible section
  // sitting near the very bottom of the page can end up parked in the
  // lower half of the viewport with no further scrolling possible to reach
  // it, so nothing else here would ever open it automatically.
  //
      ...
      <div class="mq-overlay" id="mq-lead-overlay">
        <div class="mq-modal">
          ...
          <div class="mq-modal-fields">
            <div class="mq-modal-field"><label>Your email <span style="color:#6b7280;font-weight:400">(optional)</span></label><input type="email" id="mq-lead-email" placeholder="you@email.com"/></div>
          </div>
  ...
  // COUNTERTOP FORK: simplified from the original 'c'/'b'/'ct' branching —
  // window._mqCalcCabinet no longer exists (calcCabinet is removed), so the
  // old guard `!window._mqCalcCabinet || !window._mqCalcCountertop` would
  // have made this always return null now. Only the 'ct' path survives.
  function mqCurrentLiveResult() {
    const prefix = window._mqStickyPrefix;
    if (!prefix || !window._mqCalcCountertop) return null;
    const r = window._mqCalcCountertop('ct');
  ...
  // COUNTERTOP FORK judgment call: the original standalone Countertops tab
  // ('ct') had no project-type/room selector at all, so there was nothing
  // to check a per-type toggle against — it always hardcoded a range. This
  // fork's merged single-flow layout gives 'ct' a real `mq-ct-room`
  // project-type dropdown (the same one driving price adjustments and
  // visibility filtering), so the `prefix === 'ct'` special case that used
  // to force "always show a range" is removed here — 'ct' now goes through
  // the same per-project-type `showRange` lookup 'c'/'b' always used. This
  // is a real behavior change from the original 'ct' tab (worth a second
  // look): a shop that has set a project type's "Customer sees exact
  // quote, no range" option will now have that respected for countertops
  // too, where before it was silently ignored.
  ...
  // Swaps out every "ballpark"/"customer sees this range" phrase for wording
  // that's actually true once a project type has the range toggled off —
  // this tool always shows the shop owner their own real total regardless,
  // but the label describing what the CUSTOMER sees needs to stay accurate.
  ...
  window.mqRefreshBallparkWording = function(prefix) {
    ...
    if (rangeLbl) rangeLbl.textContent = showRange ? 'Customer sees this range' : 'Customer sees this quote';
    const grandSub = document.getElementById(`mq-${prefix}-grand-sub`);
    if (grandSub) grandSub.textContent = showRange ? 'Before tax · Customer sees this range' : 'Before tax · Customer sees this quote';
  ...
  function mqSetStickyPrice(prefix, low, high, total, animate) {
    ...
    // whenever the main number above is still the customer-safe range, also
    // surface the real exact total right next to it. Skip it when the main
    // number is already the exact total (allNoRange) — nothing to add.
    ...
    if (realEl) {
      if (allNoRange) {
        ...
        if (realValEl) realValEl.textContent = CUR() + Math.round(combinedTotal).toLocaleString();
      }
    }
    if (animate && prev) {
  ...
  // COUNTERTOP FORK: simplified from the original 'c'/'b'/'ct' branching —
  // same window._mqCalcCabinet fix as mqCurrentLiveResult above (that guard
  // would have made this a silent no-op for every call now that calcCabinet
  // is removed).
  function mqUpdateLivePreview(prefix, precomputedRange) {
    let range = precomputedRange;
    if (!range) {
      if (!window._mqCalcCountertop) return;
      const r = window._mqCalcCountertop('ct');
  ...
  function mqLiveRecalcSticky() {
    ...
    try {
      ...
      // COUNTERTOP FORK: simplified from the original 'c'/'b'/'ct' branching
      // — same window._mqCalcCabinet fix as above.
      if (!range && window._mqCalcCountertop) {
        const r = window._mqCalcCountertop('ct');
        range = { low: r.low, high: r.high, total: r.total, label: 'Countertops' };
      }
      if (range) {
  ...
  async function init() {
    ...
    // the widget, including ones added later (surface cards, etc.) — no
    // need to individually wire each one.
    ...
    function mqRenderProposalModalBody() {
      ...
      body.innerHTML = `
        ...
        <label style="display:flex;align-items:center;gap:6px;font-size:13px;color:#374151;cursor:pointer;margin-bottom:14px">
          <input type="checkbox" ${state.showPrices?'checked':''} onchange="mqProposalFieldChanged('showPrices',this.checked)" style="width:16px;height:16px;flex-shrink:0;accent-color:#1a1a1a"/> Show individual item prices on this proposal <span style="font-weight:400;color:#9ca3af">(just for this one — doesn't change the template's default)</span>
        </label>

        ${(f['Deposit value']||0) > 0 ? `
        <label style="display:flex;align-items:center;gap:6px;font-size:13px;color:#374151;cursor:pointer;margin-bottom:14px">
          <input type="checkbox" ${state.waiveDeposit?'checked':''} onchange="mqProposalFieldChanged('waiveDeposit',this.checked); mqRenderProposalModalBody()" style="width:16px;height:16px;flex-shrink:0;accent-color:#1a1a1a"/> Waive the deposit for this proposal <span style="font-weight:400;color:#9ca3af">(this template normally requires one — just skip it for this one customer)</span>
        </label>` : ''}
        ...
        <div style="background:#f9fafb;border-radius:8px;padding:12px 14px;font-size:13px;color:#374151;line-height:1.8;margin-bottom:14px">
          <div style="display:flex;justify-content:space-between"><span>Subtotal</span><strong>${CUR()}<span id="mq-prop-subtotal-val">${subtotal.toFixed(2)}</span></strong></div>
          <div style="display:flex;justify-content:space-between"><span>Tax (${f['Tax percent']||0}%)</span><strong>${CUR()}<span id="mq-prop-tax-val">${taxAmt.toFixed(2)}</span></strong></div>
          <div style="display:flex;justify-content:space-between;font-size:15px;font-weight:800;border-top:1px solid #e5e7eb;margin-top:6px;padding-top:6px"><span>Total</span><span>${CUR()}<span id="mq-prop-total-val">${total.toFixed(2)}</span></span></div>
          <div style="display:flex;justify-content:space-between;color:#166534;margin-top:4px"><span>Deposit ${state.waiveDeposit?'(waived)':`(${f['Deposit type']==='Flat amount'?'flat rate':((f['Deposit value']||0)+'%')})`}</span><strong>${CUR()}<span id="mq-prop-deposit-val">${depositAmt.toFixed(2)}</span></strong></div>
    ...
    function mqBuildProposalItemsHtml(lines, showPrices, accent) {
      const rows = (lines||[]).map((l, i) => showPrices ? `
        <tr style="background:${i%2===0?'#ffffff':'#fafafa'}">
          ...
          <td style="padding:12px 14px;border-bottom:1px solid #eee;text-align:right;white-space:nowrap;font-weight:600">${CUR()}${(parseFloat(l.cost)||0).toFixed(2)}</td>
        </tr>` : `
    ...
    function mqBuildProposalItemsPlainHtml(lines, showPrices) {
      const rows = (lines||[]).map(l => showPrices ? `
        <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #e5e7eb">
          <strong>${mqEscapeHtml(l.label)}</strong><span>${CUR()}${(parseFloat(l.cost)||0).toFixed(2)}</span>
        </div>` : `
    ...
    function mqBuildProposalItemsPlainLightHtml(lines, showPrices) {
      const rows = (lines||[]).map(l => showPrices ? `
        <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #e5e7eb">
          <span>${mqEscapeHtml(l.label)}</span><span>${CUR()}${(parseFloat(l.cost)||0).toFixed(2)}</span>
        </div>` : `
    ...
    function mqBuildTotalsBoxHtml(subtotal, tax, total, deposit, accent) {
      return `<div style="background:${accent}0d;border:2px solid ${accent};border-radius:14px;padding:20px 24px;margin:24px 0">
        <div style="display:flex;justify-content:space-between;padding:5px 0;font-size:14px;color:#374151"><span>Subtotal</span><span>${CUR()}${(subtotal||0).toFixed(2)}</span></div>
        <div style="display:flex;justify-content:space-between;padding:5px 0;font-size:14px;color:#374151"><span>Tax</span><span>${CUR()}${(tax||0).toFixed(2)}</span></div>
        <div style="display:flex;justify-content:space-between;padding:10px 0;font-size:22px;font-weight:800;color:#111;border-top:2px solid ${accent};margin-top:6px"><span>Total</span><span>${CUR()}${(total||0).toFixed(2)}</span></div>
        ${(deposit||0) > 0 ? `<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;margin-top:14px;background:${accent};border-radius:10px;color:#fff;font-size:16px;font-weight:800"><span>Deposit Due Today</span><span>${CUR()}${deposit.toFixed(2)}</span></div>` : ''}
      </div>`;
    }
    ...
    function mqBuildTotalsPlainHtml(subtotal, tax, total, deposit) {
      return `<div style="margin:20px 0">
        <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:14px"><span>Subtotal</span><span>${CUR()}${(subtotal||0).toFixed(2)}</span></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:14px"><span>Tax</span><span>${CUR()}${(tax||0).toFixed(2)}</span></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:16px"><strong>Total</strong><strong>${CUR()}${(total||0).toFixed(2)}</strong></div>
        ${(deposit||0) > 0 ? `<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:14px"><strong>Deposit Due Today</strong><strong>${CUR()}${deposit.toFixed(2)}</strong></div>` : ''}
      </div>`;
    }
    ...
    function mqRenderProposalBodyTokens(bodyText, data) {
      ...
      const replacements = {
        ...
        '{subtotal}': CUR() + (data.subtotal||0).toFixed(2),
        '{tax}': CUR() + (data.tax||0).toFixed(2),
        '{total}': CUR() + (data.total||0).toFixed(2),
        '{deposit}': CUR() + (data.deposit||0).toFixed(2),
