  ...
  window.mqphSaveBulkNames = async function() {
    ...
    try {
      for (const nm of names) {
        if (cat === 'material') {
          writes.push(atCreate(LINE_ITEMS_TABLE, { shop:[shopRecord._recordId], Name:`${nm} — uppers`, Category:'material', Rate:mr.rate0||0, Unit:'per lin ft — uppers', Description:'Box material rate uppers', Active:true, 'Sort order':++sortBase }));
          writes.push(atCreate(LINE_ITEMS_TABLE, { shop:[shopRecord._recordId], Name:`${nm} — bases`, Category:'material', Rate:mr.rate1||0, Unit:'per lin ft — bases', Description:'Box material rate bases', Active:true, 'Sort order':++sortBase }));
        } else if (cat === 'door') {
          writes.push(atCreate(LINE_ITEMS_TABLE, { shop:[shopRecord._recordId], Name:nm, Category:'door', Rate:mr.rate0||0, Unit:'per lin ft upcharge', Description:'Door style upcharge', Active:true, 'Sort order':++sortBase }));
        } else if (cat === 'hinge') {
          ...
          writes.push(atCreate(LINE_ITEMS_TABLE, { shop:[shopRecord._recordId], Name:`${nm} — some drawers`, Category:'drawer', Rate:mr.rate0||0, Unit:'per lin ft upcharge', Description:'Some drawers rate (1 drawer per cabinet)', Active:true, 'Sort order':++sortBase }));
          writes.push(atCreate(LINE_ITEMS_TABLE, { shop:[shopRecord._recordId], Name:`${nm} — mostly drawers`, Category:'drawer', Rate:mr.rate1||0, Unit:'per lin ft upcharge', Description:'Mostly drawers rate (averaged 1-drawer + bank)', Active:true, 'Sort order':++sortBase }));
        }
      }
